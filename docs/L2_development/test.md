...
- `frontend/spa` は Node.js 組み込み `node:test` でプリセット構造・カタログ構築・筆算行数の純粋関数を検証する。React DOM やブラウザ E2E は対象外である。`frontend/web` には自動テストが存在しない(未確認事項参照)。

## テスト構成

`backend/tests/` には18個の `test_*.py` がある(issue #88 でリポジトリルートの `tests/` から移動)。主な責務は次のとおり。
...
`frontend/web` にはテスト・lint スクリプトは存在しない。ビルド確認のみ:
...
## 実行結果(2026-08-12、issue #88 のリポジトリ再編後に再検証)

| 検証 | 結果 |
|---|---|
| `cd backend && python3 -m pytest --collect-only -q` | 420 tests collected |
| `cd backend && python3 -m pytest -q --ignore=tests/test_nuts_calc_init.py` | 398 passed |
| `cd backend && python3 -m pytest -q`(full suite, no ignore) | 411 passed, 9 failed |
...
- `frontend/web` に `frontend/spa` 相当の `node:test`(純粋関数テスト)を追加するかどうかの方針は未確認。`drillPresets.js`/`drillCatalog.js`/`verticalLayout.js` は `frontend/spa` から複製したファイルのため、`frontend/spa` 側のテストが間接的に同じロジックをカバーしている。

...
- frontend は Node.js 組み込み `node:test` でプリセット構造と筆算行数の純粋関数を検証する(`web/frontend/src/drillPresets.test.js:1-78`、`web/frontend/src/verticalLayout.test.js:1-25`)。React DOM やブラウザ E2E は対象外である。

## テスト構成

`tests/` には15個の `test_*.py` がある。主な責務は次のとおり。

| 対象 | ファイル | 根拠・理由 |
|---|---|---|
| ReportLab CLI | `test_nuts_calc_init.py`, `test_nuts_calc_data.py`, `test_nuts_calc_cli.py` | 引数、データ、成果物を層別に検証する |
| LaTeX CLI | `test_nuts_calc_tex.py` とコマンド別 `test_nuts_calc_tex_*_generation.py` | 8コマンドの生成ロジックと `pdflatex` 成果物を検証する |
...
## 実行結果(2026-08-07、`docs/init-docs-20260807`)

| 検証 | 結果 |
|---|---|
| `python3 -m pytest --collect-only -q` | 319 tests collected |
| `python3 -m pytest -q --ignore=tests/test_nuts_calc_init.py` | 297 passed in 41.26s |
| `python3 -m pytest -q tests/test_nuts_calc_init.py` | 9 failed, 13 passed(既知 stale) |
| frontend `node --test ...` | 7 passed |
| `npm run build` | 成功(Vite 7.1.5、68 modules transformed) |
| `npm run lint` | 1失敗: `web/frontend/src/drillPresets.js:304` の全角空白を `no-irregular-whitespace` が拒否 |
...
- lint 1件をコード修正するか lint 設定で許容するかは未決定。対象は `web/frontend/src/drillPresets.js:304` と `web/frontend/eslint.config.js`。

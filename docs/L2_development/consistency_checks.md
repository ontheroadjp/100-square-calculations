...
CI 定義が存在しないため、整合性はローカルで再現可能なコマンドと実装参照で保証する。以下は `/init-docs` standalone mode 再実行(2026-08-19)の結果である。
...
| Python tests | `cd backend && python3 -m pytest -q --ignore=tests/test_nuts_calc_init.py` | 644 passed。全体は666件、除外ファイルは既知どおり9 failed / 13 passed(合算すると657 passed, 9 failed) |
...
| API | ルートと renderer 変換をソース/pytest で照合 | `POST /generate-pdf`・`POST /generate-problems`(issue #138)・`GET /renderer-info` の3本。`result_max` を含む backend テストは上記644件に含まれ成功 |
...
- docs に記載したエントリポイント8件(`backend/nuts_calc.py`, `backend/nuts_calc_tex.py`, `backend/factory.sh`, `backend/app.py`, `frontend/spa/src/main.jsx`, `frontend/web/{index,catalog,preset}.html`)の実在を確認した。`custom.html` は issue #97 で削除済みのため対象外(`docs/.ai/repo.profile.json:entrypoints` と一致)。

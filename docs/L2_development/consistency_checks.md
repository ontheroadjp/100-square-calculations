...
CI 定義が存在しないため、整合性はローカルで再現可能なコマンドと実装参照で保証する。以下は standalone `/init-docs` 再実行(2026-09-01)の結果である。
...
| Python tests | `cd backend && python3 -m pytest -q` | 1147 passed in 86.05s(`backend/pytest.ini:4` の `-n auto` による並列実行) |
...
| API | ルートと renderer 変換をソース/pytest で照合 | `POST /generate-pdf`・`POST /generate-problems`・`GET /renderer-info` の3本。既定 PDF 経路の全20コマンドと `100` 専用 JSON envelope を含む backend テストは上記1147件に含まれ成功 |

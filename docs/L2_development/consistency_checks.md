...
- `frontend/web` は `vite`/`sass` の2つのみを devDependencies に持つことを `frontend/web/package.json` で確認した。(かつて併存していた `frontend/spa` の依存バージョン(React 19.1.1、Vite 7.1.5、i18next 26.3.6 等)は `frontend/spa/package-lock.json` から確認していたが、`frontend/spa` 自体が issue #233 で削除された。)

...
CLI レベル(`--a-fraction-form`/`--b-fraction-form` を `frac -o add`/`sub` に拡張する帯分数対応、issue #112)の PDF生成・CSV・バリデーションエラーは pdflatex サブプロセス経由のため `backend/tests/test_nuts_calc_tex.py` 側でカバーする(本ファイルは pdflatex 非依存の純粋ロジックのみ)。

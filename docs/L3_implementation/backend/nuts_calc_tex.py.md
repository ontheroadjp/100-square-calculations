...
- 呼び出し先: `pdflatex`(要 LaTeX ディストリビューション、`texlive-latex-base` + `texlive-latex-extra`)。`NUTS_CALC_TEX_ENGINE=lualatex` 選択時は代わりに `lualatex`(要 `texlive-luatex`、`fonts-noto-cjk`。issue #121、詳細は `CLAUDE.md` の Local Tooling Environment を参照)を呼び出す。

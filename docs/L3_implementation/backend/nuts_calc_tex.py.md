...
- `--with-name-field`(issue #93): `build_page_header_tex(with_name_field: bool = False)`(`nuts_calc_tex.py:701-711`)が真の場合のみ、Date/Time行の直後に独立したLaTeX改行(`\\\\`)を挟んで `Name: \underline{\hspace{8cm}}` 行を追加する。`with_name_field` は `build_page_tex`(`nuts_calc_tex.py:789`)→ `build_document_tex`(`nuts_calc_tex.py:804-809`)→ `main()`(`nuts_calc_tex.py:3199-3216`、`ini.with_name_field` を2箇所の `build_document_tex` 呼び出しへ渡す)という単一経路でスレッドされる。ヘッダーは全10コマンド共通の呼び出し(`build_page_tex` 単独箇所)を通るため、`--with-bottom-answer` のようにコマンドごとの `build_X_pages` 関数へ個別配線する必要がなく、`_init()` でのコマンド限定バリデーションも行わない(全コマンドで無条件に有効)。フラグ未指定時は `with_name_field=False` がデフォルト値のまま伝播するため、既存の `Date: ... \hfill Time: ...` 行のみの出力と完全に同一(回帰なし、実機PDF比較で確認済み)。日本語ラベル(「なまえ：」)ではなく英語 `Name:` を採用した理由は後述の設計判断を参照。
...
issue の Scope 本文は日本語ラベル「なまえ：____________」を提案していたが、本ファイルは素の `pdflatex`(CJK/日本語フォントパッケージ未読み込み、プリアンブル参照)でコンパイルしており、日本語グリフを含む文字列を渡すと `Fatal error occurred, no output PDF file produced` で確実に失敗する(issue #91 の余り表記「あまり」→`\cdots` と同じ制約、詳細は上記の `ope --remainder` セクション参照)。既存ヘッダー(`Date:`/`Time:`)自体が英語表記であることとも整合させ、英語 `Name:` を採用した。この制約への根本対応(LuaLaTeX 等の日本語対応エンジンへの移行、および `docs/latex/tex_calculation_drill_layout_guidelines.md` に基づくレイアウト統一)は issue #104 として別途切り出し済み。
...
- 836e6d3 feat(#94): nuts_calc_tex.py: add evenodd/multiples/divisors number-property commands

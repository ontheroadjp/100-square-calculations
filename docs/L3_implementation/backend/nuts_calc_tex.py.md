...
- `-a`/`-b`(桁数)は `ope`/`100` と同じ `set_min_max_value()` によるレンジ変換の対象に含めた(`_init()`: `if args.command in ('ope', '100', 'lcm', 'gcd')`)。`-a`/`-b` 省略時は既定の `--a-min`/`--a-max`/`--b-min`/`--b-max`(1〜9)がそのまま使われる。`--vertical`/`--intermediate`/`--use-parentheses`/`--missing-value`/`--terms`系/`--carry-borrow`系/`--remainder`系/`--a-decimal-places`系は、いずれも既存の `_init()` バリデーションが `command != 'ope'` を理由に無条件で拒否するため、`lcm`/`gcd` を明示的に除外する追加コードは不要だった。

...
- `LatexEngineAdapter`(issue #120): プラガブルなLaTeXエンジン抽象化。`typing.Protocol` で `binary_name: str`/`build_preamble_additions() -> str`/`compile(tex_source, out_pdf_path) -> None` の3メンバーを要求する。唯一の実装 `PdflatexEngineAdapter`(`binary_name = 'pdflatex'`)は、旧 `compile_tex` 関数(以前は独立関数だった)が行っていた「一時ディレクトリで `pdflatex -interaction=nonstopmode -halt-on-error` を subprocess 実行し、生成PDFを指定パスへコピー、失敗時は `pdflatex` 出力末尾を含めて `exit(1)`」という処理をそのままメソッド化したもので、`build_preamble_additions()` は空文字列を返す(CJK対応など日本語対応エンジンが必要とする追加プリアンブルは issue #121 で別アダプターとして実装予定)。`NUTS_CALC_TEX_ENGINE` 環境変数(`LATEX_ENGINE_ENV_VAR`)で選択するエンジン名を解決する `get_latex_engine_name()`/`get_latex_engine_adapter()` は、`backend/renderers.py` の `NUTS_CALC_RENDERER`/`get_renderer_name()` パターン(未設定時は `DEFAULT_LATEX_ENGINE`(`'pdflatex'`)にフォールバックし、`LATEX_ENGINE_ADAPTERS` dict に無い値は `ValueError`)を意図的に踏襲している。`build_preamble_tex(paper_size, engine_adapter=None)` は `engine_adapter` 省略時 `PdflatexEngineAdapter()` にフォールバックする(既存呼び出し元・テストの出力を変えないため)一方、`build_document_tex` は `engine_adapter` を必須引数として要求する(`main()` 内の呼び出し元が常に解決済みインスタンスを持つため)。将来エンジンを追加する場合、新しい `LatexEngineAdapter` 実装クラスと `LATEX_ENGINE_ADAPTERS` への登録のみで足り、`build_preamble_tex`/`build_document_tex`/`main()` 側の変更は不要な設計。
...
issue の Scope 本文は日本語ラベル「なまえ：____________」を提案していたが、本ファイルは素の `pdflatex`(CJK/日本語フォントパッケージ未読み込み、プリアンブル参照)でコンパイルしており、日本語グリフを含む文字列を渡すと `Fatal error occurred, no output PDF file produced` で確実に失敗する(issue #91 の余り表記「あまり」→`\cdots` と同じ制約、詳細は上記の `ope --remainder` セクション参照)。既存ヘッダー(`Date:`/`Time:`)自体が英語表記であることとも整合させ、英語 `Name:` を採用した。この制約への根本対応(LuaLaTeX 等の日本語対応エンジンへの移行、および `docs/latex/tex_calculation_drill_layout_guidelines.md` に基づくレイアウト統一)は issue #104(トラッキング issue)として切り出し済みで、実装は #120(本ファイルの `LatexEngineAdapter` 抽象化、対応済み)→ #121(日本語対応エンジンアダプターの評価・実装)→ #122(レイアウト統一)に分解されている。

...
- `get_renderer_name()`(`renderers.py:90-107`): env 変数 `NUTS_CALC_RENDERER` を読み、未設定なら `DEFAULT_RENDERER`(`'latex'`、issue #186 で `'reportlab'` から変更)を返す。`name` が `UNAVAILABLE_RENDERERS`(`{'reportlab'}`)に含まれる場合(未設定時のフォールバックではなく明示的に指定された場合のみ到達しうる)、`RENDERER_SCRIPTS` の未知値チェックより先に「現在利用不可」の明確な `ValueError` を送出する(`nuts_calc.py` は非推奨だがコード自体は削除しない方針、issue #186)。それ以外で `RENDERER_SCRIPTS` に無い値が指定された場合は許可値一覧を含む `ValueError` を送出する。
...
- `RendererRequest`(`TypedDict`、`renderers.py:9-49`)は `num: int` フィールドを持つ(issue #138)。これは `backend/problem_generation.py`([[problem_generation.py]] 参照)専用のフィールドで、`build_command`/`run` は参照しない(`POST /generate-problems` が生成する問題数を指定するために使う)。

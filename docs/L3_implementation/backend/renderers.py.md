...
- `RendererRequest.a_digits`/`.b_digits`(issue #230): `build_command()` は他のスカラーフィールドと同じパターンで、存在すれば無条件に `--a-digits`/`--b-digits` へ変換する(値の意味を解釈しない、単純な pass-through)。`a_value`/`b_value` も同様に無条件で `--a-value`/`--b-value` へ変換する既存のパススルーのまま(`renderers.py:131-136` 付近)。どちらのフィールドがどのコマンドで意味を持つか(`a_digits`/`b_digits` は `nuts_calc_tex.DIGIT_COUNT_SHORTHAND_COMMANDS` のみ、`a_value`/`b_value` は `com`/`99`/`squ`/`pi` のみ)は `nuts_calc_tex.py` の `_init()` 側の責務であり、本ファイルはコマンドに関わらず両方を機械的に転送するだけ([[../../../../nuts_calc_tex.py]] の「`-a/--a-value` の桁数解釈を `--a-digits`/`--b-digits` へ分離した理由」参照)。
...
- `RendererRequest`: リクエスト params の型ヒント。全キー任意(`total=False`)。`carry_mode` は `Literal['required','none','mixed']` で、`CARRY_MODE_FLAGS` が順に `--carry-borrow`/`--no-carry-borrow`/`--mixed-carry-borrow` へ変換する。未知値は無視せず `ValueError` にする。`missing_value: bool` や `terms` 系も同様に保持する。`remainder_mode`(`Literal['required','none','mixed']`、issue #91)は `carry_mode` と全く同じパターンで、`REMAINDER_MODE_FLAGS` が `--remainder`/`--no-remainder`/`--mixed-remainder` へ変換し、未知値は同様に `ValueError` にする。`reducible_mode`(`Literal['required','none','mixed']`、issue #114)も同じパターンで、`REDUCIBLE_MODE_FLAGS` が `--require-reducible`/`--no-reducible`/`--mixed-reducible` へ変換する(`frac`/`mixed` の乗除算専用、[[../../../../nuts_calc_tex.py]] 参照)。`dividend_mode`(`Literal['integer','decimal','mixed']`、issue #317)も同じパターンで、`DIVIDEND_MODE_FLAGS` が `--integer-dividend`/`--decimal-dividend`/`--mixed-dividend` へ変換し、未知値は `ValueError` にする(`ope -o div` で除数が小数のときの被除数種別。5年「整数と小数の割り算」ドリル用、[[../../../../nuts_calc_tex.py]] 参照)。`with_name_field: bool`(issue #93)は `with_bottom_answer` と同じ単純な真偽フラグパターンで、真の場合のみ `--with-name-field` を追加する。
- `RendererRequest.mixed_decimal_operand_order: bool`(issue #313): `with_name_field` と同じ「真の場合のみフラグを1つ追加する」パターンで、`build_command()` は `params.get("mixed_decimal_operand_order")` が truthy のときだけ `--mixed-decimal-operand-order` を `command` に append する(`renderers.py:158-159`)。`nuts_calc_tex.py` の `ope -o mul` 専用で、`--a-decimal-places` と `--b-decimal-places` を非対称にした上で「どちらの被乗数が小数を持つか」を問題ごとにランダムに入れ替える(worksheet 内に「小数×整数」と「整数×小数」を混在させる)。他の真偽フラグ同様 `build_command()` はレンダラー名で抑止しないため、Web プリセット側が `latexOnly: true` の項目からのみ送る([[../../../../nuts_calc_tex.py]] の `### ope --a-decimal-places/--b-decimal-places` 参照)。

...
| 未対応の `command_type`(`'100'` を含む)、亜種フラグの相互排他違反、`terms_min > terms_max`、`com`/`99`/`squ`/`pi` での `a_value` 欠落・`com` の最小値未満、`frac`/`mixed`/`compare`/`simplify`/`commondenom`/`frac2dec` の digit/排他/operator/operand-kind 制約違反、`compare` の `comparison_pattern`+非既定 kind の組み合わせ、`multiples`/`divisors` の `a_min` 1未満、`multiples` の `multiples_count` 下限未満、`divfrac` の `b_min` 1未満、reportlab レンダラーへの亜種フラグ指定、その他データ層のエラー | 500 | `{ "error": "..." }` |

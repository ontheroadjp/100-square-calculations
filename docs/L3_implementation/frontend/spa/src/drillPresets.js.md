# `frontend/spa/src/drillPresets.js`

## 目的・役割

「学年(1〜6、または無学年)」を `POST /generate-pdf`(`backend/app.py`/`renderers.py`)へのリクエストパラメータにマッピングする静的な設定データ。UI ロジックを一切持たず、`GRADES`・`UNGRADED`・`CUSTOM_GRADE`・`presetsByGrade` の4つを export するのみ。
...
- **加減算の単独・混合プリセットは併存する**: 1年生は上記6カードを「加算2→減算2→混合2」の順に置く。2年生以降も加算のみ・減算のみ・加減算混合の既存3カードを保持するため、単独演算を選びたい利用者と混合練習の双方に対応する。
...
- **`nuts_calc.py`/`nuts_calc_tex.py` 間のレンダラー相違を踏まえた除外**: `backend/renderers.py` は `NUTS_CALC_RENDERER` 環境変数でどちらのスクリプトにも切り替えられ、両者が同一の CLI サーフェスを持つ前提で実装されている。しかし調査の結果、3箇所で挙動が食い違うことが判明し、それぞれ別issueとして追跡している:
...
- `nuts_calc.py`/`nuts_calc_tex.py`/`backend/app.py` 側にパラメータの許可リストバリデーションが薄いため(`docs/L3_implementation/specification_summary.md` 既知の制約)、ここで不正な組み合わせを作らないよう注意する。特に上記のレンダラー相違(issue #41/#42/#43)が解消されるまでは、該当する組み合わせを追加しないこと。

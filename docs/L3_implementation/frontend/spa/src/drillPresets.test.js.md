# `frontend/spa/src/drillPresets.test.js`
...
比較カードについて、4年生の同分母・同分子、5年生の異分母と各発展カードが `compare` と正しい `comparison_pattern` を使うことを確認する。さらに全カードのタイトル・説明キーが日英翻訳辞書に存在することを検証する（`frontend/spa/src/drillPresets.test.js:160-191`）。

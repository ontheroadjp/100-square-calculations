...
### 5年生の総合問題(複数ソース混在ワークシート)(issue #368, frac2dec/dec2frac追加issue #383)

5年生に `review` カテゴリを新設し、1項目 `g5-review`(`titleKey: 'menu_g5_review_title'`「5年の総合問題」)を置いた(親 issue #358 の全学年 `review` 展開の一子。g1-review #365 / g2-review #366 / g4-review #367 と同型)。学習指導要領 第5学年 A「数と計算」の複数単元 ―― 小数×小数、小数÷小数(わり進み)、異分母・真分数のたし算・ひき算、分数×整数・分数÷整数、分数→小数の変換、小数→分数の変換 ―― を1枚に混在させる。`CATEGORY_ORDER` は既に `'review'` を先頭に持つため、`catalog.js`/`pcMakeFlow.js` は無変更。`review` は `grade5` オブジェクト内では最後のカテゴリキー(`decimal`/`four-operations`/`fraction`/`number-sense` の後)として追加した(grade1〜4 の `review` が常に最後のカテゴリキーである慣例に合わせる)。

- `settings: []`、`supportLevel: 'full'`、`latexOnly: true`。`examples` は静的(`['3.6×2.4', '9.0÷2.5', '2/3+3/5', '3/5×4', '3/4 → 0.75', '0.6 → 3/5']`)。
- `buildParams()` は引数を取らず `{ command_type: 'review', shuffle: true, sources: [...6件...] }` を返す。`sources` 各要素は既存の5年生メニュードリルのオプションをそのまま流用し `num: 1` を付けたもの ―― (1) `ope` mul `a_digits:2` `b_digits:2` `a_decimal_places:1` `b_decimal_places:1`(= `g5-decimal-mul`)、(2) `ope` div 同レンジ + `divide_through:true`(= `g5-decimal-div` の余り「わり進み」分岐)、(3) `frac` add/sub `numerator_digits:1` `denominator_digits:1` `different_denominators:true` `proper_result:true`(= `g5-fraction-add`/`g5-fraction-sub` の denominator `different` + numberKind `fraction` 分岐)、(4) `mixed` mul/div `a_kind:['fraction']` `b_kind:['int']` `numerator_digits:1` `denominator_digits:1` `reducible_mode:'mixed'`(= `g5-fraction-mul-int`/`g5-fraction-div-int`)、(5) `frac2dec` `numerator_digits:1` `denominator_digits:1`(= `g5-frac2dec`)、(6) `dec2frac`(引数無し、= `g5-dec2frac`)。`num` は相対ウェイトで [[../../../../backend/three_layer_renderer.py]] `_generate_review_pdf` が全問題数(10/20/30)へ按分する。等ウェイト×6。
...
- **分数⇔小数の変換を追加した理由(issue #383)**: issue #368 の起票時点の Background 引用にはこの単元が含まれておらず(見落とし)、代わりに issue #369 が誤って第6学年の内容として `g6-review` に持ち込んでいた。学習指導要領上「小数、整数を分数の形に直したり、大きさを比べたりすること」は第5学年 A「数と計算」の内容であり、標準ドリル `g5-frac2dec`/`g5-dec2frac` が既に存在するため、`g6-review` から削除した上でこちらへ移設した。
- `command_type: 'review'` は `POST /generate-problems` 非対応(g1/g2/g3/g4-review と同じ)。
- `drillPresets.test.js` の `g5-review` 専用テストを6 source(旧4 source + frac2dec/dec2frac)に更新(issue #383、[[./drillPresets.test.js]] 参照)。文言は [[./strings.ja.json]] の `menu_g5_review_desc` に分数⇔小数の変換を追記。

### 6年生の総合問題(複数ソース混在ワークシート)(issue #369, frac2dec/dec2frac削除issue #383)

6年生に `review` カテゴリを新設し、1項目 `g6-review`(`titleKey: 'menu_g6_review_title'`「6年の総合問題」)を置いた(親 issue #358 の全学年 `review` 展開の最後の子。g1-review #365 / g2-review #366 / g4-review #367 / g5-review #368 と同型)。学習指導要領 第6学年 A「数と計算」の単元 ―― 分数×分数・分数÷分数、分数・小数を含む四則混合 ―― を1枚に混在させる。`CATEGORY_ORDER` は既に `'review'` を先頭に持つため、`catalog.js`/`pcMakeFlow.js` は無変更。`review` は `grade6` オブジェクト内では `fraction` の後、最後のカテゴリキーとして追加した(grade1〜5 の `review` が常に最後のカテゴリキーである慣例に合わせる)。

- `settings: []`、`supportLevel: 'full'`、`latexOnly: true`。`examples` は静的(`['3/5×7/9', '2/3+0.5×4']`)。
- `buildParams()` は引数を取らず `{ command_type: 'review', shuffle: true, sources: [...2件...] }` を返す。`sources` 各要素は既存メニュードリルのオプションをそのまま流用し `num: 1` を付けたもの ―― (1) `frac` mul/div `numerator_digits:1` `denominator_digits:1` `proper_operands:true` `reducible_mode:'mixed'`(= `g6-fraction-mul`/`g6-fraction-div` の統合)、(2) `mixed` add/sub/mul/div `mixed_operators:true` `terms:3` `a_kind`/`b_kind:['fraction','decimal','int']` `numerator_digits:1` `denominator_digits:1` `decimal_places:1`(= `g6-fraction-decimal-mixed`)。`num` は相対ウェイトで [[../../../../backend/three_layer_renderer.py]] `_generate_review_pdf` が全問題数(10/20/30)へ按分する。等ウェイト×2。
...
- **分数⇔小数の変換(frac2dec/dec2frac)を削除した理由(issue #383)**: issue #369 起票時の Background が学習指導要領を誤って引用しており、この単元は実際には第5学年 A「数と計算」の内容(「小数、整数を分数の形に直したり…」)で、第6学年の新出内容(分数の乗法及び除法のみ)には含まれない。6年生の grade メニューには対応する standalone ドリルも存在しないため、`g1〜g5-review` が守る「各 source は同学年の individual ドリルと一致させる」原則に反していた。5年生には元々 standalone ドリル `g5-frac2dec`/`g5-dec2frac` が存在するため、`g5-review` へ移設し、ここからは削除した。
- **6年生スコープ外の4項目を source 化しない理由**: `g6-int-mul-fraction`/`g6-int-div-fraction`(整数×分数・整数÷分数)/`g6-fraction-muldiv-mixed`/`g6-fraction-four-ops` は issue #369 の確定済み Scope 箇条書き(分数×分数・分数÷分数、分数・小数を含む四則混合の2単元のみ)に含まれないため source から除外した(g5-review が `g5-approx-quotient` を除外したのと同じ判断)。
- `command_type: 'review'` は `POST /generate-problems` 非対応(g1/g2/g3/g4/g5-review と同じ)。
- `drillPresets.test.js` の `g6-review` 専用テストを2 source(`frac` mul/div 統合・`mixed` 四則混合のみ)に更新し、frac2dec/dec2frac の assertion を削除(issue #383、[[./drillPresets.test.js]] 参照)。文言は [[./strings.ja.json]] の `menu_g6_review_desc` から分数⇔小数の変換の記述を削除。issue #358(親)が全学年の review 展開を追跡しており、issue #369 で1〜6年生すべてに `review` カテゴリが揃った。

...
- `RendererRequest`(`TypedDict`、`renderers.py:9-48`): リクエスト params の型ヒント。全キー任意(`total=False`)。`carry_mode` は `Literal['required','none','mixed']` で、`CARRY_MODE_FLAGS` が順に `--carry`/`--no-carry`/`--mixed-carry` へ変換する。未知値は無視せず `ValueError` にする(`renderers.py:26,62-66,143-150`)。`missing_value: bool` や `terms` 系も同様にLaTeX専用パラメータとして保持する。
...
- `--vertical`、`--use-parentheses`、`--missing-value`、`--terms`系、`--mixed-operators`、`--carry`/`--no-carry`/`--mixed-carry` は `nuts_calc.py` に存在しない。これらを持つリクエストはLaTeXレンダラーに限定する([[../../../../nuts_calc.py]] 参照)。

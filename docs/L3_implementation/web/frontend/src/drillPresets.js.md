...
- **`written`(筆算)は `nuts_calc.py --vertical` が対応する組み合わせに厳密に限定している**: `--vertical` は `ope` コマンドの `add`/`sub` と、掛ける数が1桁の `mul` のみサポートし、`div`/`mix`/複数桁 `mul` は CLI 側のバリデーションで `exit(1)` になる(`docs/L3_implementation/nuts_calc.py.md` 参照)。そのため `written` に含めているのは各学年の `add`/`sub`(桁数を学年に応じて増やして近似)と、3年生の `mul`(2桁×1桁、これは近似ではなく実際にサポートされている組み合わせそのもの)のみ。4〜6年生の掛け算・わり算の筆算は、複数桁乗数対応(issue #10)・筆算(長除法)対応(issue #11)が実装されるまで意図的に含めていない。

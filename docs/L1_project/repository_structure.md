...
- `backend/renderers.py`: `NUTS_CALC_RENDERER` env 変数(デフォルト `latex`、issue #186 で `reportlab` から変更)で `nuts_calc.py`/`nuts_calc_tex.py` を切り替えて呼び出す、Flask 非依存の純粋関数群(issue #36)。明示的な `reportlab` 指定は issue #186 で「利用不可」エラーになり到達不能(コード自体は削除していない)。`RENDERER_SCRIPTS` はスクリプトパスを `Path(__file__).resolve().parent`(=`backend/`)基準で解決する(issue #88 で `web/backend/` からの移動に伴い repo-root 基準から変更)。

...
#              worksheets in PDF format. It utilizes the 'python nuts_calc_tex.py' Python script
...
# 3.  **Problem Generation (Mental Arithmetic)**: Calls 'python nuts_calc_tex.py' multiple times
...
#
# Dependencies:
# -   'python nuts_calc_tex.py': The Python script responsible for generating the PDF worksheets.
#     Ensure 'python nuts_calc_tex.py' is executable and located in the same directory or
#     accessible via the system's PATH. Unlike the removed nuts_calc.py (issue #232),
#     nuts_calc_tex.py renders via LaTeX and requires a working pdflatex/lualatex on PATH.
...
function _basic {
    ...
    for long in 20 45; do
        ...
        for size in a3 a4; do
            ...
            python nuts_calc_tex.py ${size} aBc -a 2 -b 1 -o add -c ${c} -r ${r} -p ${page} \
...
function _kuku() {
    for size in a3 a4; do
        ...
        for dan in 1 2 3 4 5 6 7 8 9; do
            python nuts_calc_tex.py ${size} 99 -a ${dan} -c 3 -r 9 -p ${page} \
...
function _kuku_descend() {
    for size in a3 a4l; do
        [ "${size}" == "a3" ] && page=1 || page=1
        for dan in 1 2 3 4 5 6 7 8 9; do
            python nuts_calc_tex.py ${size} 99 -a ${dan} -c 3 -r 9 -p ${page} --reverse \
...
function _kuku_random() {
    for size in a3 a4l; do
        [ "${size}" == "a3" ] && page=1 || page=1
        for dan in 1 2 3 4 5 6 7 8 9; do
            python nuts_calc_tex.py ${size} 99 -a ${dan} -c 3 -r 9 -p ${page} --shuffle \
...
function _kuku_all_mix() {
    for size in a3 a4l; do
        [ "${size}" == "a3" ] && page=5 || page=10
        python nuts_calc_tex.py ${size} ope --a-min 1 --a-max 9 --b-min 1 --b-max 9 \

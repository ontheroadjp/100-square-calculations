...
@dataclass
class ReviewProblem:
    """One slot of a multi-source 'review' (総合) worksheet (issue #140).

    A review worksheet interleaves problems generated by several different
    drills onto one page. Each ReviewProblem wraps one underlying problem
    dataclass (`payload`, e.g. an OpeProblem or FractionProblem) together
    with the `kind` key selecting which number-free slot formatter renders
    it. `index` is the 1..N slot number assigned after every source is
    concatenated and optionally shuffled, so the caller sets it rather than
    the per-source generator.
    """
...
def build_review_slot_content_tex(problem: ReviewProblem, show_answer: bool) -> str:
    """Render the number-free body for one review slot by delegating to the
    slot formatter for its `kind` (issue #140).

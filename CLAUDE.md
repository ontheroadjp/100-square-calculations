...
- `frontend/web` (lightweight static multi-page site — `index.html`/`catalog.html`/`preset.html` — Vite + Sass + KaTeX, Japanese only): `npm run build` succeeds (verified 2026-08-18, 3 HTML entries). Its two `node:test` files are run directly and 30 tests passed; no npm test/lint scripts are configured. Its grade→category→menu-item model diverged from `frontend/spa` in issue #98.

...
*   **Web UI**: a frontend (`frontend/spa`, a React SPA, or `frontend/web`, a lightweight static multi-page site) → Flask backend (`backend/app.py`) → `backend/renderers.py` → `subprocess` call to `nuts_calc.py` (default) or `nuts_calc_tex.py` (via `NUTS_CALC_RENDERER=latex`) → generated PDF is streamed back to the browser. The backend holds no drill-generation logic of its own; it only translates form input into CLI arguments shared by both renderers. Both frontends call the same two endpoints (`POST /generate-pdf`, `GET /renderer-info`) and are independent npm projects rather than sharing a package, since a future repo split was anticipated; `frontend/web`'s `verticalLayout.js` still mirrors `frontend/spa`'s copy, but its `drillPresets.js` has its own grade -> category -> menu-item data model (issue #98) and no longer duplicates `frontend/spa`'s version.

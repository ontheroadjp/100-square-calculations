...
## Features
*   **Diverse Problem Types**: Generate worksheets for integer arithmetic, complements, 100-square tables, multiplication tables, square numbers, mental arithmetic, exact fraction arithmetic, fraction comparison, and number properties (even/odd, multiples, divisors) through the LaTeX renderer.
...
**Experimental**: `nuts_calc_tex.py` is independent from ReportLab and implements thirteen commands: the seven ReportLab-compatible commands, the LaTeX-only `frac`, `mixed`, and `compare` commands, and the LaTeX-only `evenodd`, `multiples`, and `divisors` number-property commands. It also owns decimal, written-calculation, and carry-aware drill behavior. Web cards using these features are renderer-gated because `nuts_calc.py` does not implement them. See `docs/L3_implementation/backend/nuts_calc_tex.py.md`.

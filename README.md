...
*   **Renderer-owned drill logic.** The web backend does not duplicate worksheet-generation rules; the default path dispatches requests to `three_layer_renderer.py`, which calls `nuts_calc_tex.py`'s data and presentation APIs in-process. The CLI/subprocess renderer path remains only as a source-level emergency rollback switch.
